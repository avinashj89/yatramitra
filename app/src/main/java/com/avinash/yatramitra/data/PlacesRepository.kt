...
object PlacesRepository {
    ...
    // Real production endpoints. Every function below takes these as overridable parameters
    // (defaulting to these constants) purely so unit tests can redirect calls to a local
    ...
    internal const val NOMINATIM_BASE_URL = "https://nominatim.openstreetmap.org"
    internal const val OVERPASS_BASE_URL = "https://overpass-api.de"
    ...
    suspend fun searchPlaces(query: String, baseUrl: String = NOMINATIM_BASE_URL): List<PlaceSuggestion> =
        withContext(Dispatchers.IO) {
            if (query.trim().length < 2) return@withContext emptyList()
            try {
                val encoded = URLEncoder.encode(query.trim(), "UTF-8")
                val url = "$baseUrl/search?q=$encoded&format=json&limit=5&addressdetails=0"
    ...
    suspend fun geocode(place: String, baseUrl: String = NOMINATIM_BASE_URL): PlaceSuggestion? {
        return searchPlaces(place, baseUrl).firstOrNull()
    }
    ...
    suspend fun geocodeOrThrow(place: String, baseUrl: String = NOMINATIM_BASE_URL): PlaceSuggestion? =
        withContext(Dispatchers.IO) {
            val query = place.trim()
            if (query.length < 2) return@withContext null
            val encoded = URLEncoder.encode(query, "UTF-8")
            val url = "$baseUrl/search?q=$encoded&format=json&limit=1&addressdetails=0"
            val request = Request.Builder().url(url).header("User-Agent", USER_AGENT).build()
            client.newCall(request).execute().use { response ->
    ...
    suspend fun reverseGeocode(lat: Double, lon: Double, baseUrl: String = NOMINATIM_BASE_URL): String =
        withContext(Dispatchers.IO) {
            try {
                val url = "$baseUrl/reverse?lat=$lat&lon=$lon&format=json&zoom=14"
                val request = Request.Builder().url(url).header("User-Agent", USER_AGENT).build()
                client.newCall(request).execute().use { response ->
    ...
    suspend fun findNearbyPitstop(
        ...
        try {
            ...
            val request = Request.Builder().url(url).header("User-Agent", USER_AGENT).build()
            client.newCall(request).execute().use { response ->
                if (!response.isSuccessful) return@withContext reverseGeocode(lat, lon, nominatimBaseUrl)
                val body = response.body?.string() ?: return@withContext reverseGeocode(lat, lon, nominatimBaseUrl)
                val obj = org.json.JSONObject(body)
                val elements = obj.optJSONArray("elements") ?: return@withContext reverseGeocode(lat, lon, nominatimBaseUrl)
                ...
                reverseGeocode(lat, lon, nominatimBaseUrl)
            }
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
